...
// Portrait scenes (height > width) — shrink to 50% width so they don't tower.
...
function buildScenes() {
  ...
  lineKeys.forEach((lineIdx, sceneIdx) => {
    ...
    const isPortrait = PORTRAIT_SCENES.has(sceneIdx % CINEMATIC_PHOTOS.length);
    const sceneW = isPortrait ? SCENE_W * 0.5 : SCENE_W;
    const xDir = sceneIdx % 2 === 0 ? -1 : 1;
    const sceneX = vW / 2 + xDir * ZIGZAG_X - sceneW / 2;
    ...
    sceneEl.style.cssText = `left:${sceneX}px;top:${sceneY}px;width:${sceneW}px;`;

    const photoWrap = document.createElement('div');
    photoWrap.className = 'scene-photo-wrap';

    // Try mp4 first, then jpeg/jpg/png/webp, then Unsplash fallback
    const videoSrc = `photos/scene-${sceneIdx}.mp4`;
    ...
    vid.onerror = () => {
      ...
      let extIdx = 0;
      const tryImg = () => {
        if (extIdx >= imgExts.length) return; // keep Unsplash fallback
        const src = `photos/scene-${sceneIdx}.${imgExts[extIdx++]}`;
        const t = new Image();
        t.onload = () => { photoEl.src = src; };
        t.onerror = tryImg;
        t.src = src;
      };
      tryImg();
